...
output_file = 'evolution1a.csv' # file to output population fitness
...
def new_population(population,outfile):   # this function now has two arguments
    new_p = []
    sum_f = sum_fitness(population)
    print(sum_f)
    outfile.write(str(sum_f) + '\n')      # output written to file as well as screen
...
def simulation(generations):
    output = open(output_file,'a')           # open the output file for writing
    ...
    output.close()                           # remember to close the output file at the end
    return population
